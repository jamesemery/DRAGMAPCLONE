...
public:
  ...
  const map::SeedChain& chain() const {assert(chain_); return *chain_;}
  void setChain(const map::SeedChain& c) {assert(!chain_ || &c == chain_); chain_ = &c;}
...
public:
  ...
  bool             hasOnlyRandomSamples() const { return
    (at(0).isUnmapped() || at(0).hasOnlyRandomSamples()) &&
    (at(1).isUnmapped() || at(1).hasOnlyRandomSamples());}
  bool             isExtra() const { return
    (at(0).isUnmapped() || at(0).isExtra()) &&
    (at(1).isUnmapped() || at(1).isExtra());}
